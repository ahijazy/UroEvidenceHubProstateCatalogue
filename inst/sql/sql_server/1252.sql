...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (35201068,715808,947884)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (35201068)
...
JOIN 
(
  select ENDS.person_id, min(drug_exposure_start_date) as era_start_date, DATEADD(day,15, ENDS.era_end_date) as era_end_date
  from
  (
