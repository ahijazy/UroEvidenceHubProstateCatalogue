...
( 
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (35603551,902727,43526934)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (35603551,902727,43526934)
  and c.invalid_reason is null

) I
LEFT JOIN
(
  select concept_id from @vocabulary_database_schema.CONCEPT where concept_id in (45775578,41267222)
UNION  select c.concept_id
  from @vocabulary_database_schema.CONCEPT c
  join @vocabulary_database_schema.CONCEPT_ANCESTOR ca on c.concept_id = ca.descendant_concept_id
  and ca.ancestor_concept_id in (45775578)
  and c.invalid_reason is null
...
JOIN 
(
  select ENDS.person_id, min(drug_exposure_start_date) as era_start_date, DATEADD(day,45, ENDS.era_end_date) as era_end_date
  from
  (
    ...
    (
      --cteEndDates
      select PERSON_ID, DATEADD(day,-1 * 45,EVENT_DATE) as END_DATE -- unpad the end date by 45
      FROM
      (
				...
				(
					...
					-- add the end dates with NULL as the row number, padding the end dates by 45 to allow a grace period for overlapping ranges.
					select PERSON_ID, DATEADD(day,45,DRUG_EXPOSURE_END_DATE), 1 as EVENT_TYPE, NULL
					FROM #drugTarget D
				) RAWDATA
      ) E
